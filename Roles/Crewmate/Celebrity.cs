...
internal class Celebrity : RoleBase
{
    ...
    public override bool GlobalKillFlashCheck(PlayerControl killer, PlayerControl target, PlayerControl seer)
    {
        ...
        if (!ImpKnowCelebrityDead.GetBool() && seer.GetCustomRole().IsImpostorTeamV3()) return false;
        if (!NeutralKnowCelebrityDead.GetBool() && seer.GetCustomRole().IsNeutralTeamV3()) return false;
    ...
    public override void OnMurderPlayerAsTarget(PlayerControl killer, PlayerControl target, bool inMeeting, bool isSuicide)
    {
        ...
        if (inMeeting)
        {
            ...
            {
                if (!ImpKnowCelebrityDead.GetBool() && pc.GetCustomRole().IsImpostorTeamV3()) continue;
                if (!NeutralKnowCelebrityDead.GetBool() && pc.GetCustomRole().IsNeutralTeamV3()) continue;
    ...
    public override void OnOthersMeetingHudStart(PlayerControl targets)
    {
        ...
        {
            if (!ImpKnowCelebrityDead.GetBool() && targets.GetCustomRole().IsImpostorTeamV3()) continue;
            if (!NeutralKnowCelebrityDead.GetBool() && targets.GetCustomRole().IsNeutralTeamV3()) continue;
