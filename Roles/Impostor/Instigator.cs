...
internal class Instigator : RoleBase
{
    ...
    {
        if (exiled == null || !exiled.Object.IsCrewmate()) return;
        ...
        foreach (var playerVote in votedForExiled)
        {
            ...
            if (crewPlayer == null || !crewPlayer.IsCrewmate() || crewPlayer.IsAnySubRole(x => !x.IsCrewmateTeamV2()) || !crewPlayer.IsAlive()) continue;
